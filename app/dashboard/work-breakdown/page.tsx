...
interface WorkItem {
  ...
  isExpanded?: boolean;
  isEditing?: boolean;
  shouldDelete?: boolean; // 新增属性，用于标记是否需要删除
}
...
export default function WorkBreakdownPage() {
  ...
  // 添加子工作项
  const addChildWorkItem = (parentId: string, level: number) => {
    const newItem: WorkItem = {
      id: `temp-${Date.now()}`,
      name: `新${level + 1}级工作项`,
      description: "",
  ...
  // 移除 expandAllChildren, expandAllChildrenRecursive, collapseAllChildren, collapseAllChildrenRecursive, toggleChildrenExpand 函数
